...
df_sko <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Korea, South") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_au <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Australia") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_ge <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Germany") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_sp <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Spain") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_fr <- world %>% dplyr::filter(type == "confirmed", Country.Region == "France") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
  dplyr::mutate(fr = cumsum(cases)) %>%
  dplyr::filter(fr > 100)  %>%
  dplyr::select(-cases, -date)
...
df_it <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Italy") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_sw <- world %>% dplyr::filter(type == "confirmed", Country.Region == "Switzerland") %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(cases = sum(cases)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(date) %>%
...
df_trajectory_can <- df_trajectory_can %>% 
...
# canada_region<- can_p[can_p$date_report==max(can_p$date_report),]
# canada_region$a_cum<-canada_region$c_cum - canada_region$d_cum - canada_region$r_cum
# names(canada_region)[4]<-"Cumulative Cases"
# names(canada_region)[6]<-"Cumulative Deaths"
# 
# levels(canada_region$province)<-c("Alberta","British Columbia","Manitoba","New Brunswick", "Newfoundland and Labrador", "Nova Scotia","Nunavut","Northwest Territories", "Ontario","Prince Edward Island","Québec", "Repatriated","Saskatchewan","Yukon")
# 
# `%>%` <- magrittr::`%>%`
# canada_map_region <- rnaturalearth::ne_states(country = "Canada", returnclass = "sf") %>%
#   dplyr::select(province = name) %>%
#   dplyr::left_join(canada_region[,c("province","Cumulative Cases","Cumulative Deaths")], by = c("province" = "province"))
...
plotly::plot_ly(data = df_trajectory) %>%
  ...
  plotly::add_lines(x = ~ index,
                    y = ~ us,
                    line = list(color = us_color, width = 2),
                    name = "United States") %>%
   plotly::add_lines(x = ~ index,
                    y = ~ sp,
                    line = list(color = sp_color, width = 2),
                    name = "Spain") %>%
   plotly::add_lines(x = ~ index,
                    y = ~ it,
                    line = list(color = it_color, width = 2),
                    name = "Italy") %>%
   plotly::add_lines(x = ~ index,
                    y = ~ fr,
                    line = list(color = fr_color, width = 2),
                    name = "France") %>%
   plotly::add_lines(x = ~ index,
                    y = ~ ge,
   ...
   plotly::add_lines(x = ~ index,
                    y = ~ uk,
                    name = "United Kingdom",  line = list(width = 2,color = "#9db847")) %>%
   plotly::add_lines(x = ~ index,
                    y = ~ sw,
                    line = list(color = sw_color, width = 2),
                    name = "Switzerlands") %>%
   plotly::add_lines(x = ~ index,
                    y = ~ au,
...
# 
# ```{r}
# canada_region[canada_region$province != "Repatriated",c(2,4,9,6,7,8)] %>%
#   DT::datatable(rownames = FALSE,
#                 colnames = c("Region","Cumulative Cases", "Cumulative Active Cases","Cumulative Deaths", "Cumulative Recovered", "Total tests completed"), options = list(order = list(2, 'desc'), searching = FALSE), filter = 'none')
# ```
```
...
### Canada COVID-19 Data as of `r data_date$value`
